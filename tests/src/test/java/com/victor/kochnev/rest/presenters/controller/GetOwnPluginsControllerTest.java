...
class GetOwnPluginsControllerTest extends BaseControllerTest {
    private static final String MY_PLUGIN_ENDPOINT = "/plugin/own";
    private static UUID OWNER_ID1;
    private static UUID OWNER_ID2;
    private static UUID PLUGIN_ID1;
    private static UUID PLUGIN_ID2;
    private static UUID PLUGIN_ID3;
    private static UUID PLUGIN_ID4;
    private static UUID PLUGIN_ID5;
    @Autowired
    ...
    void prepareDb() {
        OWNER_ID1 = userRepository.save(UserEntityBuilder.defaultBuilder().build()).getId();
        userForRequest = userRepository.findById(OWNER_ID1).get();

        PLUGIN_ID1 = pluginRepository.save(PluginEntityBuilder.persistedPostfixBuilder(1)
                .name("Name1")
                .ownerUser(userRepository.findById(OWNER_ID1).get())
        ...
        OWNER_ID2 = userRepository.save(UserEntityBuilder.persistedPostfixBuilder(2).build()).getId();
        PLUGIN_ID5 = pluginRepository.save(PluginEntityBuilder.persistedPostfixBuilder(5)
                .ownerUser(userRepository.findById(OWNER_ID2).get())
                .build()).getId();
